...
To run the Techpreneurs site locally, there are a few things to set up.
...
_If you're looking to work on the Techpreneurs site and want access to our accounts, please get in touch_

### Install Node.js and MongoDB

You'll need node 0.10.x and npm 1.3.x installed to run Techpreneurs. The easiest way is to download the installers from [nodejs.org](http://nodejs.org).
...
### Setting up your copy of Techpreneurs
...
### Running Techpreneurs

Once you've set up your configuration, run `node keystone` to start the server.

By default, [Keystone](http://keystonejs.com) will connect to a new local MongoDB database on your localhost called `techpreneurs`, and create a new Admin user that you can use to log in with using the email address `user@keystonejs.com` and the password `admin`.
...
When it's all up and running, you should see the message `Techpreneurs is ready on port 3000` and you'll be able to browse the site on [localhost:3000](http://localhost:3000).
