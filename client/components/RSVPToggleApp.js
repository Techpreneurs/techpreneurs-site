...
var RSVPToggleApp = React.createClass({
	...
	renderRSVPToggle: function() {
		...
		var attendingText = this.state.rsvp.attending ?  "You're coming!" : "Yes";
		var notAttendingText = this.state.rsvp.attending ? 'Cancel' : "Can't make it?";
		return (
			<div>
				{this.renderWelcome()}
				<div className="rsvp-button">
					<div id="next-meetup" data-id={this.state.meetup._id} className="form-row meetup-toggle">
						...
						<div className="col-xs-4">
							<button type="button" onClick={this.toggleRSVP.bind(this, false)} className={"btn btn-lg btn-block btn-default btn-decline js-rsvp-decline " + notAttending}>{notAttendingText}</button>
						</div>
					</div>
				</div>
			</div>
